...
import { useCreateProduct } from "../../../services/products-services";

export default function AddProductModal({ onClose, onSave, categories }) {
  console.log("From add modal: ", categories);
  ...
  const { handleSubmit, loading } = useCreateProduct(
    {
      productName,
      description,
      basePrice,
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl relative flex flex-col max-h-[90vh]">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className={clx("w-full  text-white py-3 rounded-xl mt-4  transition-colors duration-300", loading ? "bg-blue-300" : "bg-blue-600 hover:bg-blue-700")}
          >
            {loading ? (
